...
| HIGH-3 | `presence` not in CI; no automated test gate | **High** | ✅ **RESOLVED** (Audit Findings Impl) — added a `test` job to `.github/workflows/deploy.yml` running the deterministic pure suites + the 14 platform invariants; both deploy jobs now `needs: test` (a red gate blocks deploy). Verified locally: 38 pure suites pass, 4 integration-only skipped. *(Auto-deploying the presence **function** via CI stays deferred — the verified prod path is manual `supabase-go`; the standard CLI has a known segfault on it.)* | Done |

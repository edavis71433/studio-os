...
### Presence CMS Phase 1 — Hardening track (M1–M10) — IN PROGRESS: 4 of 10 implemented (40%) · M4 idempotency awaiting owner migration activation
...
- ✅⏳ **M4 — Publish idempotency + cooldown** — **Implementation ✅ COMPLETE · Activation ⏳ PENDING** (Jul 9 2026). `lib/publish_guard.ts` + `handlePublish`: Idempotency-Key replay → in-flight 409 → 60s cooldown (429+Retry-After); client path only (admin/agency/restore/scheduler unaffected); cross-tenant key isolated by `(site_id,key)`. guard 21/21 + tenant_isolation 11/11; 88/88 sweep; deployed — **cooldown is LIVE**. Idempotency **implemented but awaiting activation**: owner applies migration `0073_publish_idempotency.sql` (on the owner pre-launch checklist / [[pre-launch-reminders]]).
- ⏳ **M5 (NEXT active) → M10** — remaining in the approved order: M5 deploy robustness · M6 media hardening · M7 snapshot retention GC · M8 preview hardening · M9 client UX safety · M10 ops/load/DR. Not reordered.

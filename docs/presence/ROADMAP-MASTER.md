...
- 🟡 **BR-1 · Brand & Identity System — PARTIAL (Phase BR-1).** Goal: every customer interaction feels like one premium Studio OS experience, with the **Brand Kit as the single source of truth**. ✅ *Done:* (1) the **one branded email shell** (`lib/email_brand.ts` `brandEmailShell` — table layout + inline styles, no external assets, contrast-safe accent via `readableOnWhiteText`); **`sendEmail` now wraps every email through it** (Studio OS default, or the customer's Brand Kit) — **no second email engine**; the customer-facing lead-notify + one-tap-approval emails carry the **site's brand** (accent CTAs), and all other emails get a consistent default shell. (2) **Design-language audit** across every customer-facing surface — Studio App / Client App / Website / Customers / Files / Analytics / Inbox / Preview / Launches all load the shared `shell.css` + canonical accent `#5b3fa0`; **fixed** `connections-callback.html` (the one page still on the retired warm palette → converged to canonical tokens). email_brand 10/10; 77/77 regression; deployed. ⛔ *Remains (kept under BR-1 — not split):* logo image in emails (needs a public logo URL, e.g. from the live deploy); **PDF/invoice** branding (**no PDF engine exists** — reports are print-HTML today; a branded PDF path is net-new and would be its own build); client-facing **Reports** branding; branded **exports**; shared **empty/loading/success/error** state components as a design-system set (each page has its own today); per-customer **Client App** chrome branding (the operator Studio App intentionally uses the canonical Studio OS palette). `approve.html`/`portal.html` are transitional pages that use the canonical accent inline but not `shell.css` (visually consistent; token de-dup = FD-15).

...
- ➕ **Launches (FD-T7, owner request):** change the template / stage a redesign in a parallel named version while the live site keeps running + stays hotfixable, then promote at a moment — AEM-Launches/Contentful-Launch/Sanity-Releases class. Bounded one-live-lane + one-launch-lane (general branching stays excluded). First slice: FD-T8 template-switch staged preview. Creatives additions: FD-T9 logo→brand-kit extraction, FD-T10 stock photos, FD-T11 crop/focal, FD-T12 section ordering as data.
  - **Status (Phase T-BLOCKS-3):** ✅ **FD-T8 DONE** — `/preview?template=` renders any registered look with the real draft content, **nothing persisted**, contract-checked (`preview.ts`); the look chooser uses it. ✅ **FD-T11 (crop/focal) DONE** (DS-5, mig 0057). ✅ **FD-T12 (section ordering as data) DONE** (DS-2). ⏳ **FD-T7 full Launches lane** (a *named parallel draft* alongside the live/working draft, promotable atomically) is the large remaining piece — it changes the one-draft model, so it needs owner sign-off on the approach before build (proposed conforming design: a second named draft that renders through the **same** `renderSnapshot` pipeline and promotes as one atomic publish — no second renderer/pipeline). ⏳ **FD-T9 logo→brand-kit** (extract palette from an uploaded logo) and ⏳ **FD-T10 stock photos** remain — FD-T10 needs an image *source*, which is an external origin, so it's owner/provider-gated (a bundled curated set would conform; a live stock API would not sit on the published site — Part 4).
...
| **FD-T7 full Launches lane** (named parallel draft + atomic promote) | ⏳ **remaining — owner sign-off on the approach first** (changes the one-draft model; conforming design proposed above; no second pipeline) |
| **FD-T9 logo→brand-kit** (palette from logo) | ⏳ remaining (creative; buildable — deterministic color extraction) |
| **FD-T10 stock photos** | ⏳ remaining — **owner/provider-gated** (a live stock API is an external origin — Part 4; a bundled curated set would conform) |

...
### Presence CMS Phase 1 — Hardening track (M1–M10) — IN PROGRESS: 3 of 10 complete (30%)
...
- ✅ **M3 — Draft-version hash** — DONE (Jul 9 2026). `lib/draft_hash.ts` (`computeDraftHash`) hashes the canonical serialized draft content minus the timestamp; compute-on-read via the ONE serializer (no migration, no publish change); `draft_hash` on `/site`. Tests 10/10; 87/87 sweep; deployed.
- ⏳ **M4 (NEXT active) → M10** — remaining in the approved order: M4 publish idempotency+cooldown · M5 deploy robustness · M6 media hardening · M7 snapshot retention GC · M8 preview hardening · M9 client UX safety · M10 ops/load/DR. Not reordered.

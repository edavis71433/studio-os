...
### Presence CMS Phase 1 — Hardening track (M1–M10) — IN PROGRESS: 4 of 10 complete (40%)
...
- ✅ **M4 — Publish idempotency + cooldown** — DONE (Jul 9 2026). `lib/publish_guard.ts` + `handlePublish`: Idempotency-Key replay → in-flight 409 → 60s cooldown (429+Retry-After); client path only (admin/agency/restore/scheduler unaffected); cross-tenant key isolated by `(site_id,key)`. guard 21/21 + tenant_isolation 11/11; 88/88 sweep; deployed — **cooldown live; idempotency pending owner migration 0073** (see [[pre-launch-reminders]]).

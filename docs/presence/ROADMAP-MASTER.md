...
2. ⛔ **OWNER-CONFIG (unchanged, not code).** stripe-webhook must be deployed + registered (`--no-verify-jwt`, Stripe endpoint added, `STRIPE_WEBHOOK_SECRET` set, events checkout.session.completed + payment_intent.*), and a **test-mode run-through** done before go-live. Without it NO payment (audit OR invoice) flips to paid. On [[pre-launch-reminders]].
...
### Presence CMS Phase 1 — Hardening track (M1–M10) — IN PROGRESS: 2 of 10 complete (20%)
...
- ⏳ **M3 (NEXT active) → M10** — remaining in the approved order: M3 draft-version hash · M4 publish idempotency+cooldown · M5 deploy robustness · M6 media hardening · M7 snapshot retention GC · M8 preview hardening · M9 client UX safety · M10 ops/load/DR. Not reordered.

...
- ➕ **FD-T15 · Block-authoring UI in the editor** (discovered Phase T-BLOCKS) — the block library's storage/validation/render/schema and the `/settings` endpoint are shipped and proven (24/24), so blocks are fully functional and render correctly; the remaining piece is the **owner-facing control in `presence.html`** to add/fill/reorder blocks visually. *Why it belongs:* it's the last step that makes the realized block library usable by a non-technical owner without the API. *Recommended priority:* **V1 if launching small-business-broad** (blocks are the "genuinely good, not just correct" layer), **V1.1 if restaurant-first**. Browser-QA-gated (can't be verified headless).
- ➕ **FD-T16 · Realize blocks in the `editorial` + `restaurant-classic` templates** (discovered Phase T-BLOCKS) — the shared `renderSiteBlocks` is template-agnostic and wired into `business-classic` (the default); adopting it in the other two families is a small mechanical change. *Why it belongs:* block parity across every template family. *Recommended priority:* **V1.1.**
- ➕ **FD-T17 · Media-bearing blocks** (discovered Phase T-BLOCKS) — v1 blocks are text-only (deterministic, no media plumbing); gallery / team-photo / before-after / video blocks want a media reference through the serializer's `ref()`. *Why it belongs:* completes the visual half of the block catalog. *Recommended priority:* **V1.1.**

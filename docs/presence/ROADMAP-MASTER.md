...
- ➕ **FD-T18 · Team-member photo picker in the UI** (discovered Phase T-BLOCKS-2) — the team block renders member photos when a `media_id` is set, but the current team editor is a text (pipe-delimited) control, so photos aren't yet pickable there. *Why it belongs:* completes the team block's visual half. *Recommended priority:* **V1.1** (small, browser-QA-gated).

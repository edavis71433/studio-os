...
- ➕ **FD-T2.1 · Lazy template code-splitting** (discovered Phase T-REGISTRY) — flip the registry loaders from eager static imports to per-version dynamic `import()` so hundreds of template versions don't all bundle/parse on cold start. *Why it belongs:* the only remaining half of FD-T2 (the indexed catalog + integrity guard + loader isolation shipped); the loader boundary is already in place, so this is a localized change. *Why staged:* it makes `renderSnapshot` async (the frozen sync render contract + sync `render_test` rely on it being sync) and needs a live-publish to verify the edge-function bundles computed dynamic imports correctly — validate during a live publish (Gold Master), not headless. *Recommended priority:* **V1.1 / when the template-version count grows** (no customer impact at 3 templates).
...
| **FD-T2 lazy/indexed registry** | ✅ **indexed half DONE (Phase T-REGISTRY)** — `lib/render.ts` now has a memoized, metadata-first `templateIndex()`, render resolution isolated behind a single **loader boundary** (`getTemplate` stays sync + memoized), and a **`registryIntegrity()` guard** (a mis-keyed template can't ship silently at scale). registry_test 7/7; render 28/28 (frozen sync contract intact). The remaining **lazy dynamic-import code-splitting** is **FD-T2.1** (appended) — it would make `renderSnapshot` async (breaking the frozen sync render contract + sync render_test) and needs a live-publish bundling validation, so it's staged behind the loader boundary, not deferred on "premature" grounds. |

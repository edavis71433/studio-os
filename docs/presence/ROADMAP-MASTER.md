...
- 🟡 **BR-1 · Brand & Identity System — PARTIAL (Phase BR-1).** Goal: every customer interaction feels like one premium Studio OS experience, with the **Brand Kit as the single source of truth**. ✅ *Done:* (1) the **one branded email shell** (`lib/email_brand.ts` `brandEmailShell` — table layout + inline styles, no external assets, contrast-safe accent via `readableOnWhiteText`); **`sendEmail` now wraps every email through it** (Studio OS default, or the customer's Brand Kit) — **no second email engine**; the customer-facing **lead-notify + one-tap-approval + client-invite + lead-follow-up + domain-notice** emails carry the **site's brand** (accent CTAs via `loadEmailBrand`), and Studio-OS→customer product/billing notices get the consistent default shell. (2) **Cross-surface design-language audit COMPLETE** — every signed-in product surface (Studio App / Client App / Website / Customers / Files / Analytics / Inbox / Preview / Launches / Design Studio / Content Library) loads the shared `shell.css` + canonical accent `#5b3fa0`; `approve.html` confirmed canonical + theme-aware (light default, dark `@media`, `data-theme` overrides). **Fixed** the one real drift: `connections-callback.html` (retired warm/brown palette → canonical tokens). (3) **Branded export cover** — `lib/export_cover.ts` extends the EXISTING `buildExport()` (`SiteExport.cover_html`) with a self-contained, on-brand cover (business name, brand accent, page/photo/brand/domain summary) derived from the **same** `brandFromKit` the emails use — **reuses the export pipeline; no PDF engine, no second export system**. email_brand 10/10 + export_cover 7/7; 78/78 regression; deployed.
  ...
  ⛔ *Remains (kept under BR-1 — NOT split):*
...
- ➕ **FD-T2.1 · Lazy template code-splitting** — ⚠️ *this ID was a split of FD-T2 I created before your no-split rule; the same remaining work is now captured under **FD-T2 (PARTIAL)** above. Awaiting your call: fold this into FD-T2 and remove this ID, or keep it. I won't delete it without approval.* Flip the registry loaders from eager static imports to per-version dynamic `import()` so hundreds of template versions don't all bundle/parse on cold start. *Why it belongs:* the only remaining half of FD-T2; the loader boundary is already in place, so this is a localized change. *Why staged:* it makes `renderSnapshot` async (the frozen sync render contract + sync `render_test` rely on it being sync) and needs a live-publish to verify the edge-function bundles computed dynamic imports correctly — validate during a live publish, not headless. *Recommended priority:* **V1.1 / when the template-version count grows** (no customer impact at 3 templates).
...
### AUD-1 — Paid-Audit Checkout Fulfillment (QUEUED — return to it "tonight" per Eric, Jul 9 2026)
The paid-audit tiers now reach Stripe (new `public_audit_checkout` in clever-api, deployed staging+prod; buy-audit.html + audit.html rebuilt plain-language, committed LOCAL/unpublished). End-to-end trace surfaced fulfillment gaps to close before go-live:
1. **[High] Align the new handler to the existing webhook contract.** stripe-webhook fulfils audits via `metadata.order_id` → flips an `audit_orders` row to paid (the admin payments panel at clever-api:9929 reads `audit_orders`). The fast handler only sets `metadata[kind]=audit` + details in Stripe metadata — it does NOT create an `audit_orders` row or pass `order_id`. So no order row appears in the admin panel and website/notes never reach the DB (only Stripe). Fix: insert a pending `audit_orders` row (name/email/website/notes/tier/amount) then pass `metadata[order_id]`. Table already exists (0000_baseline).
2. **[High/config] stripe-webhook must be deployed + registered** (`--no-verify-jwt`, endpoint added in Stripe, `STRIPE_WEBHOOK_SECRET` set, events checkout.session.completed + payment_intent.*). Without it NO payment (audit OR invoice) flips to paid or hits the stripe_payments ledger. Verify at go-live. Also on [[pre-launch-reminders]].
3. **[Med] No proactive "new paid audit" alert.** Free-tool leads email Eric + show in the brief; a PAID audit only lands in the admin panel + Stripe's own email. Consider notify-Eric on paid order (reuse the audit_lead email pattern).
4. **[Med] Success-page copy is invoice-specific.** payment-success.html says "your invoice is already marked paid… your project keeps moving" — wrong for a one-off audit buyer. Make audit-aware or branch by metadata.kind.

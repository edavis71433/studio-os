...
- **Wave 11 — a11y keyboard + focus-trap:** shared enhancer in `shell.js` (all shell pages) + inlined into portal + console — makes every clickable `[onclick]` element keyboard-operable (role=button/tabindex/Enter-Space, survives re-renders via MutationObserver) and traps Tab in a visible dialog. This resolves the single most pervasive a11y defect (clickable `<div>/<tr>`) across the whole app in one move. Plus `files.html` nested-button restructured.
- **Wave 12 — accessibility statement:** `accessibility.html` expanded from 3 sentences to an ADA-defensible statement (WCAG 2.1 AA target, what-we-do, tested-with, known limitations, feedback mechanism + 3-day response commitment + alt-format offer, review date).
- **Wave 13 — console perf:** killed the RHX 40-health-calls-per-keystroke storm (warm-only-once) + debounced 10 search inputs (160ms).

**Deliberately deferred (with justification):**
- The 19k console's *deep structural refactor* — one shared `esc`/`att`/`callEdge` across the 27 module IIFEs, removing the dead Opportunities (`~5008-5720`) + duplicate Approvals (`15400-15528`) modules, and per-search focus-restore. These are behaviour-changing edits to the owner's live operator tool, which has **no test harness**; doing them blind risks regressions. They should be done against a staging copy. (The a11y enhancer, the health-storm fix, and the debounces above ARE applied because they're additive/isolated and low-risk.)
- **§6 legal completeness** (terms California ARL / severability / venue; privacy CCPA-CPRA-GDPR-retention-cookie-table) — needs owner/counsel review, not an engineering guess. The public-page metadata/secrecy issues around them ARE handled.
- **§7 remaining dead-code + retirement** → folds into **P2-G** (legacy `clever-api`, `portal.html` service sections, orphan pages).
- Per-page modal `role=dialog`/`aria-modal` additions where a modal doesn't yet declare them (the global trap only engages once a dialog is marked) — incremental.

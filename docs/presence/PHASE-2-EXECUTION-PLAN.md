...
| **P2-B** ✅ | **App Shells, Navigation & Shared Foundations** *(engineering done Jul 9)* | `presence.html` adopts the ONE shared shell + `ddsToast` (editor JS incl. M9 If-Match untouched); `buildNav` verified as the one surface-separating nav; **`portal-workspace.html` retired** (reversible redirect, zero inbound links); **`portal.html` KEPT** (sign-in + 7 capabilities deferred to P2-C/D/E/Phase-8 — matrix `P2B-CAPABILITY-PRESERVATION-MATRIX.md`). p2b_shell 23/23; sweep 95/0; live browser/mobile/AT QA deferred. | WS1, WS4 (nav/discoverability) |
| **P2-C** ✅ | **Sales & Customer Lifecycle** *(engineering done Jul 9)* | One coherent multi-tenant chain: contact→deal(stage ladder)→proposal(signed accept)→contract(version-integrity sign)→**idempotent convert→provisionForSignup→onboarding**. New `lib/sales_lifecycle.ts` (pure) + `routes/sales.ts` (coherent `/sales/*`) + migration 0074 (5 site-scoped tables) + Studio `pipeline.html` on the shell. Reuses provisioning + first-run + email + the HMAC link idiom. sales_lifecycle 33/33 + sales_routes 29/29 + e2e (staging); sweep 98/0. **Migration 0074 owner-apply pending.** Legacy sales routes flagged for P2-G. | WS2 (cohesion), the lead→customer bridge |

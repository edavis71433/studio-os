...
| # | Item | Owner | Why |
|---|---|---|---|
| B1 | ✅ **Stripe PRODUCTION CONFIG COMPLETE (Jul 9 2026)**: live `STRIPE_SECRET` (rotated `rk_live_` restricted key, Checkout Sessions:write) set in the PROD project `qksstlqzbhesadrrofgn` (secrets are project-wide → covers presence + clever-api + stripe-webhook); **production checkout VERIFIED** — returns a real `cs_live_` Stripe URL; found + fixed a real routing bug (`public_audit_checkout` was missing from `PUBLIC_ROUTES` → 403 `unknown_route`, blocking every audit sale); audit purchase flow unblocked; deployed both envs. **Remaining pre-launch Stripe tasks (OWNER, not engineering blockers):** (a) send a Stripe **webhook test event** → confirm 200; (b) ~~**publish** buy-audit/audit/payment-success pages (deployment fence)~~ ✅ **LIVE since Jul 11 2026** (fence lifted, 344 commits pushed); (c) one **test-mode subscription signup**; (d) one **test-mode invoice payment**. | 🧑🤖 | Billing engine live + verified; remaining = owner run-throughs~~+ the push~~ (pushed Jul 11). |
...
| B4 | **Backups + a real restore test** on both Supabase projects. *(Re-verified open Jul 12 2026 — no drill run; `/system/health` now exposes a `BACKUPS_VERIFIED` activation flag to set once the drill passes.)* | 🧑🔒 | You cannot sell what you can't recover |
| B5 | **Reconcile migration history** so a single migration applies without the hold-back ritual. *(Re-verified open Jul 12 2026 — prod migrations still apply via manual SQL-editor paste runbooks: `APPLY-0081-0085-prod.sql`, now `APPLY-0086-0092-prod.sql`.)* | 🤖 | The current ritual is manual and one typo from a prod mistake |
| ~~B6~~ | ✅ **DONE (Jul 9 2026 · Phase 1 M2)** — audited every `svc()` query that consumes a request-supplied id; all client routes scope to the caller's `site.id`; added 3 defense-in-depth `&site_id=eq.` hardenings + `tenant_isolation_test.mjs` (9/9). No RLS-bypass found; global sentinel confirmed operator-only. Record: [PHASE1-SECURITY-AUDIT.md]. | 🤖 | The one RLS-bypass class — closed + regression-guarded. |
| B7 | **Secret rotation confirmed** (Netlify token, any password shown in chat); never reset `edavis7143@yahoo.com`. *(Re-verified open Jul 12 2026 — and the rotation list GREW: the publish-root purge found an `_internal/` runbook that printed a shared secret, publicly fetchable until the fix is pushed; rotate that secret too — see the Round-5 commit's OWNER TODO.)* | 🧑 | Historical exposure |
| B8 | **Basic monitoring + alerting** (function errors, failed deploys, DB health) + a one-page incident runbook. 🟢 *~SHIPPED by the Jul 11–12 audit campaign:* cross-region watchdog (`supabase/ops/watchdog-cron.sql`, staging→prod `/system/health` → email) + **`ops_errors` ledger with a global catch** in the presence function (function-error alerting) + an **honest `/system/health`** (cron-staleness + failure-alarm + `failures_last_24h`), on top of the weekly digest / domain-expiry watch / lead nudge. The **incident runbook now exists** ([INCIDENT-RUNBOOK.md](INCIDENT-RUNBOOK.md), Jul 12). *Residue (Jul 12):* an **external non-email monitor** (email-only alerting fails silently if email fails) + the **restore drill** (see B4). | 🧑🤖 | Operating blind is not acceptable for paid users |
...
| P2 | **Cookie Policy + consent flow** — 🟢 *consent half SHIPPED (Jul 12 2026, audit campaign):* **consent-gated analytics** (`analytics.js` — one shared include, **default-deny**: no GA and no analytics cookies until the visitor explicitly allows; replaced every legacy inline GA snippet). Remaining: the Cookie **Policy** document/legal review. | 🤖🔒 | Public-launch legal baseline |

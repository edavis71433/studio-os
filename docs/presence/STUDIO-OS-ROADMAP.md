...
- **P2-C — Sales & Customer Lifecycle → RE-FRAMED into P2-C1 + P2-C2** *(foundation-first split, Jul 9 2026; see `P2C-FOUNDATION-FIRST-SPLIT.md`)*. The whole chain is BUILT + deployed; the split is about review/hardening boundaries, nothing rebuilt.
  - ⏳ **P2-C1 — Lead Management & CRM Foundation** *(built → VALIDATION-first)* — lead→CRM→opportunity→pipeline. `presence_contacts` + `presence_deals` (**lead+opportunity unified by stage — FROZEN model**) + `presence_deal_events` + `/sales/*` (search/filter/paginate) + `pipeline.html` + the `leads.html` "→ Deal" bridge; reuses the `crm.html` lens. **Remaining = pure validation, in order:** owner applies migration `0074` → verify runtime on staging → live tenant-isolation → browser/mobile/AT QA → confirm architecture stable. **No optimization (dedup, CRM↔Pipeline link, polish) until validation passes.** *(next active)*

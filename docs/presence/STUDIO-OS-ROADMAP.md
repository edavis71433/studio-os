...
**Current active milestone:** **Phase 2 · P2-C2 — Sales Closing Workflow** (P2-A ✅ · P2-B ✅ · **P2-C1 ✅ COMPLETE Jul 9** — engineering gates all green on staging: runtime 16/16 · tenant isolation 8/8 · regression 33/33+44/44 · architecture stable). **✅ Data model FROZEN:** lead+opportunity = one `presence_deals` by stage. **Human browser/mobile/keyboard/screen-reader QA relocated to Phase 6 Gold Master** (runs once against the integrated product; carried forward via `P2C1-HUMAN-QA-PACKAGE.md`, not waived). P2-C2 is built (proposals/contracts/convert/onboarding) → validate it on the same engineering gate. **Do not begin P2-C2 implementation without explicit approval.** Optimizations (lead-dedup, CRM↔Pipeline link) remain deferred until evidence justifies. (P2-A ✅ · P2-B ✅ engineering done Jul 9 — shell adoption + capability-preservation matrix + duplicate-portal retirement; live browser/mobile/AT QA of the presence.html shell layout deferred to the live pass. See `PHASE-2-EXECUTION-PLAN.md` + `P2B-CAPABILITY-PRESERVATION-MATRIX.md`). Phase 1 ✅ COMPLETE. Phase 2 = rebuild worthy capabilities multi-tenant + optimize (DoD) + retire `clever-api` after verified parity (data is disposable). Do not begin P2-C implementation without explicit approval.
...
- **P2-C — Sales & Customer Lifecycle → RE-FRAMED into P2-C1 + P2-C2** *(foundation-first split, Jul 9 2026; see `P2C-FOUNDATION-FIRST-SPLIT.md`)*. The whole chain is BUILT + deployed; the split is about review/hardening boundaries, nothing rebuilt.
  ...
  - ⏳ **P2-C2 — Sales Closing Workflow** *(built → its own review; **NEXT ACTIVE**)* — proposals (signed accept) · contracts (version-integrity signing) · idempotent convert→`provisionForSignup`→guided onboarding (with convert-time login invite, `?next=` landing, agency-portfolio link) · public-endpoint rate limits · deep-review fixes (guarded rollback; claim-first convert). Validate on the same engineering gate as P2-C1 (runtime · tenant isolation · regression · architecture-stable); human QA folds into Phase 6. `lib/sales_lifecycle.ts` (pure) + `routes/sales.ts` (`/sales/*`) + migration `0074` (5 site-scoped tables, deny-all RLS) + Studio `pipeline.html` on the shell (folded under Customers). Reuses provisioning/first-run/email/HMAC-link. sales_lifecycle 33/33 · sales_routes 29/29 · e2e (staging); sweep 98/0. **⚠️ Migration `0074` owner-apply pending.** Live browser QA deferred.

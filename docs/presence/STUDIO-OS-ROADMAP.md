...
- ✅ **P2-C — Sales & Customer Lifecycle** *(engineering done Jul 9)* — one coherent multi-tenant chain: contact→deal (bounded stage ladder)→proposal (line items + signed accept link)→contract (version-integrity signing)→**idempotent convert→`provisionForSignup`→existing guided onboarding**. **Post-review hardening + cohesion:** convert-time login invite (set-password), public-endpoint rate limits, real in-dialog forms; deep-review fixes (guarded rollback — never delete a reused customer; claim-first convert — no double-provision race); **cohesion seams closed** — convert links an agency operator's new customer into their portfolio (`presence_agency_clients`, Studio↔Client loop), the invite lands straight in guided onboarding (`?next=`), and `leads.html` promotes an inquiry into a deal (`source_submission_id`, no re-typing); + selectable edition on convert, `expected_close` date validation, valid-stage-only UI. `lib/sales_lifecycle.ts` (pure) + `routes/sales.ts` (`/sales/*`) + migration `0074` (5 site-scoped tables, deny-all RLS) + Studio `pipeline.html` on the shell (folded under Customers). Reuses provisioning/first-run/email/HMAC-link. sales_lifecycle 33/33 · sales_routes 29/29 · e2e (staging); sweep 98/0. **⚠️ Migration `0074` owner-apply pending.** Live browser QA deferred.

...
- **P2-C — Sales & Customer Lifecycle → RE-FRAMED into P2-C1 + P2-C2** *(foundation-first split, Jul 9 2026; see `P2C-FOUNDATION-FIRST-SPLIT.md`)*. The whole chain is BUILT + deployed; the split is about review/hardening boundaries, nothing rebuilt.
  - ⏳ **P2-C1 — Lead Management & CRM Foundation** *(built → VALIDATION-first; **validation tooling READY**)* — lead→CRM→opportunity→pipeline. `presence_contacts` + `presence_deals` (**lead+opportunity unified by stage — FROZEN model**) + `presence_deal_events` + `/sales/*` (search/filter/paginate) + `pipeline.html` + the `leads.html` "→ Deal" bridge; reuses the `crm.html` lens. **All validation prep is done — turnkey:** `P2C1-VALIDATION-RUNBOOK.md` (ordered gate + apply-0074 steps + human browser/mobile/AT checklist), `scripts/validate-p2c1.mjs` (one-command runner + gate board), and the `sales_foundation_e2e` (runtime) + `sales_tenant_isolation_e2e` (live two-tenant) harnesses (skip w/o creds). **The ONLY blocker is owner applying migration `0074`;** then run the runner with staging creds → do the human QA. No optimization until the gate passes. *(next active)*

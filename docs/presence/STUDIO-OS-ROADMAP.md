...
**Current active milestone:** **Phase 2 · P2-D — Projects, Communication & Service Delivery — IN PROGRESS** (P2-A ✅ · P2-B ✅ · P2-C ✅). **P2-D-1 FOUNDATION landed + validated live on staging (Jul 10):** `presence_projects`/`presence_milestones`/`presence_tasks`/`presence_project_events` (migration `0075`) + pure `lib/service_delivery.ts` + `routes/projects.ts` + idempotent convert→project handoff. Gates: pure 38/38 · structural 38/38 · project e2e 21/21 · project tenant isolation 9/9 · full sweep 106/0/4 · typecheck clean · P2-C regression intact (closing 21/21, foundation 16/16, isolation 8/8). Deployed staging+prod (`/projects/*` dormant on prod until owner applies `0075`). Matrix + architecture: `P2D-CAPABILITY-MATRIX.md`; gate `scripts/validate-p2d.mjs`; evidence in the matrix doc. **REMAINING P2-D increments (sequenced, NOT started):** deliverables(files overlay)+approvals · communication(messages/notes/notifications read-state) · surveys+support · client reporting + Studio/Client UI · full 16-step lifecycle gate. **P2-D is NOT complete.** Key reuse decisions frozen in the matrix (build ON media store, relationship-notes, portal-feed, forms substrate, Approved-Plan spine + contract content_hash, shared shell/roles/shares — no parallel systems). Do not begin P2-E.
...
- 🔨 **P2-D — Projects, Communication & Service Delivery** *(IN PROGRESS)* — projects · tasks · milestones · files · messaging · notifications · surveys · approvals · support · reporting. **The service-delivery layer is net-new in `presence`** (it lived only in disposable single-tenant `clever-api`); every *supporting* primitive already exists and is reused (media store · relationship-notes · portal-feed · forms · Approved-Plan spine + contract content_hash · shared shell/roles/shares). Matrix + frozen architecture: `P2D-CAPABILITY-MATRIX.md`.
  - ✅ **P2-D-1 Foundation (Jul 10)** — `presence_projects`/`milestones`/`tasks`/`project_events` (mig `0075`) + `lib/service_delivery.ts` (pure) + `routes/projects.ts` + idempotent convert→project handoff (`presence_deals.created_project_id` UNIQUE) + feature-gate `projects`→relationship + reviewer read-allow. Studio manages; client_reviewer reads only `client_visible`. Gates all green (pure 38/38 · structural 38/38 · e2e 21/21 · isolation 9/9); deployed staging+prod; **prod `0075` owner-apply pending**.
  - ⏳ **P2-D-2..6 (NOT started):** deliverables(files overlay)+approvals · communication(messages/notes/notifications read-state) · surveys+support · client reporting + Studio/Client UI · full 16-step lifecycle gate + legacy parity/deprecation map. **P2-D is not complete until the full-lifecycle gate passes.**

...
**Current active milestone:** **Phase 2 · P2-C1 — Lead Management & CRM Foundation (built → production-quality hardening)**. The full sales chain is built + deployed but **re-framed foundation-first** into P2-C1 (Lead/CRM/Opportunity/Pipeline — harden + verify) and P2-C2 (proposals/contracts/convert/onboarding — parked for its own review). See `P2C-FOUNDATION-FIRST-SPLIT.md`. **P2-C1 remaining is verification, not build:** owner applies migration `0074` → staging e2e + browser/mobile/AT QA + a lead-dedup guard; bless the unified-deal model. (P2-A ✅ · P2-B ✅.) Do not start P2-C2 or P2-D without explicit approval. (P2-A ✅ · P2-B ✅ engineering done Jul 9 — shell adoption + capability-preservation matrix + duplicate-portal retirement; live browser/mobile/AT QA of the presence.html shell layout deferred to the live pass. See `PHASE-2-EXECUTION-PLAN.md` + `P2B-CAPABILITY-PRESERVATION-MATRIX.md`). Phase 1 ✅ COMPLETE. Phase 2 = rebuild worthy capabilities multi-tenant + optimize (DoD) + retire `clever-api` after verified parity (data is disposable). Do not begin P2-C implementation without explicit approval.
...
- **P2-C — Sales & Customer Lifecycle → RE-FRAMED into P2-C1 + P2-C2** *(foundation-first split, Jul 9 2026; see `P2C-FOUNDATION-FIRST-SPLIT.md`)*. The whole chain is BUILT + deployed; the split is about review/hardening boundaries, nothing rebuilt.
  - ⏳ **P2-C1 — Lead Management & CRM Foundation** *(built → now to production quality)* — lead→CRM→opportunity→pipeline. `presence_contacts` + `presence_deals` (lead+opportunity unified by stage) + `presence_deal_events` + `/sales/*` (search/filter/paginate/dedupe) + `pipeline.html` + the `leads.html` "→ Deal" bridge; reuses the `crm.html` lens. **Remaining = production-quality gate:** owner applies migration `0074`, then staging e2e + browser/mobile/AT QA + a lead-dedup guard; bless the unified-deal model. *(next active)*

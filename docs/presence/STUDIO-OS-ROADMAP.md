...
**Current active milestone:** **Phase 2 · P2-C1 — Lead Management & CRM Foundation (built → VALIDATION-first)**. The full sales chain is built + deployed but **re-framed foundation-first** into P2-C1 (Lead/CRM/Opportunity/Pipeline) + P2-C2 (proposals/contracts/convert/onboarding — parked). See `P2C-FOUNDATION-FIRST-SPLIT.md`. **✅ Data model FROZEN:** lead+opportunity = one `presence_deals` by stage (no separate tables, no sync). **P2-C1 next = PURE VALIDATION (prove first, optimize second):** owner applies migration `0074` → verify runtime on staging → live tenant-isolation → browser/mobile/AT QA → confirm architecture stable. **Optimizations (lead-dedup, CRM↔Pipeline link, polish) are DEFERRED until after validation.** (P2-A ✅ · P2-B ✅.) Do not start P2-C2/P2-D without approval. (P2-A ✅ · P2-B ✅ engineering done Jul 9 — shell adoption + capability-preservation matrix + duplicate-portal retirement; live browser/mobile/AT QA of the presence.html shell layout deferred to the live pass. See `PHASE-2-EXECUTION-PLAN.md` + `P2B-CAPABILITY-PRESERVATION-MATRIX.md`). Phase 1 ✅ COMPLETE. Phase 2 = rebuild worthy capabilities multi-tenant + optimize (DoD) + retire `clever-api` after verified parity (data is disposable). Do not begin P2-C implementation without explicit approval.
...
- **P2-C — Sales & Customer Lifecycle → RE-FRAMED into P2-C1 + P2-C2** *(foundation-first split, Jul 9 2026; see `P2C-FOUNDATION-FIRST-SPLIT.md`)*. The whole chain is BUILT + deployed; the split is about review/hardening boundaries, nothing rebuilt.
  - ⏳ **P2-C1 — Lead Management & CRM Foundation** *(built → **automated gate PASSED on staging**; human QA pending)* — lead→CRM→opportunity→pipeline. `presence_contacts` + `presence_deals` (**lead+opportunity unified by stage — FROZEN model, held under real use**) + `presence_deal_events` + `/sales/*` + `pipeline.html` + `leads.html` "→ Deal" bridge; reuses `crm.html`. **✅ Live validation (staging, Jul 10):** foundation runtime **16/16** + tenant isolation (two workspaces) **8/8** + 33/33 + 44/44. Evidence: `P2C1-VALIDATION-RESULTS.md`. **Finding:** `0074` was NOT actually applied to staging (verification caught it) → applied to **staging** via Management API; **prod `0074` still owner-apply.** **Remaining:** **step 4 human browser/mobile/AT QA** (accounts prepared; blocked on a staging-served UI — the app pages hardcode prod + are unpushed per fence) → then step 5 architecture-stable. No optimization until step 4/5 pass. *(next active)*
  - ⏳ **P2-C2 — Sales Closing Workflow** *(built → parked for its own review)* — proposals (signed accept) · contracts (version-integrity signing) · idempotent convert→`provisionForSignup`→guided onboarding (with convert-time login invite, `?next=` landing, agency-portfolio link) · public-endpoint rate limits · deep-review fixes (guarded rollback; claim-first convert). Reviewed on the same gate after P2-C1 is proven. `lib/sales_lifecycle.ts` (pure) + `routes/sales.ts` (`/sales/*`) + migration `0074` (5 site-scoped tables, deny-all RLS) + Studio `pipeline.html` on the shell (folded under Customers). Reuses provisioning/first-run/email/HMAC-link. sales_lifecycle 33/33 · sales_routes 29/29 · e2e (staging); sweep 98/0. **⚠️ Migration `0074` owner-apply pending.** Live browser QA deferred.
- ⏳ **P2-D — Projects, Communication & Service Delivery** *(next active)* — projects · tasks · milestones · files · messaging · notifications · surveys · approvals · support · reporting.

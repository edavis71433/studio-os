...
- 🟡 **Lone holdout:** `presence.html` (the editor) still uses the legacy `--dds-*`/serif shell-token system. Unifying it is the one real design-debt item — but it is the most complex *live* surface, so it needs **visual QA in a browser**, not a blind headless restyle. Recommended as the first task of a browser-QA session, not shipped unverified.

...
27. **G27 — PHILOSOPHY: per-element style overrides** [ST-1 deep]. Adobe allows style choice per component instance everywhere [DR-8]; our G4 delivers the curated version. Anything beyond named curated variants (custom classes, raw CSS on the no-code path) conflicts with the deny-by-default token allowlist; gated Developer-Mode CSS already serves pros. Recommendation: reject beyond G4. **—**
...
**Wave 1 — highest owner value, S/M, philosophy-clean (10 items):** G1 radius-everywhere · G2 un-gate hero/header presets · G3 focal-point-in-galleries · G4 per-block style variants · G5 scheduled publish windows · G6 shareable draft preview link · G7 page operations suite · G8 block where-used · G9 visual publish diff · G10 unpublish/offline. Rationale: G1–G3 close the inventory's own known gaps (owner-visible controls that currently half-work); G4 imports Adobe's single best philosophy-compatible idea; G5–G10 finish publish/version UX where our pipeline is already 80% built.

...
1. **Stripe (you have the account):** confirm the prod key is **live-mode** (`sk_live_…` — the health check can't see mode) and that the Stripe dashboard has a webhook endpoint → `https://qksstlqzbhesadrrofgn.supabase.co/functions/v1/presence/commerce/webhook` with events `checkout.session.completed` + `customer.subscription.*`, and that its signing secret matches the `STRIPE_WEBHOOK_SECRET` already set.
2. **Resend (you have the account):** dashboard → Domains → **verify your sending domain** (add the SPF + DKIM records Resend shows — the Foundations plan flow can help once live). Optionally set `EMAIL_FROM` (e.g. `hello@davisdigitalstudio.com`) and `OPS_ALERT_EMAIL`. **The Phase-RL lifecycle emails ride on this.**
3. **PITR + a restore drill** — Supabase dashboard (both projects; prod at minimum).
4. **External uptime/alert monitor** — point anything (UptimeRobot-class) at `/system/health?secret=…` (the secret lives in **Vault → `presence_scheduler_secret`** in both projects — retrieve it there; it's deliberately not written in any doc).
5. **Google OAuth redirect URIs** for connected-platform go-live.

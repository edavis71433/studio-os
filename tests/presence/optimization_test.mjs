...
{
  ok('composability: the ONE registry now runs 30 providers (15 + 13 + connected + industry)', PROVIDERS.length === 30, `providers=${PROVIDERS.length}`);
  const full = runProviders(BASE, PROVIDERS);
  ok('composability: M9.0 and M10 providers run together, all recorded', full.record.length === 30 && full.record.every((r) => r.ok));
...
if (SB && SR && ANON) {
  ...
  ok('integration: the evidence run succeeds with all 30 providers', run.status === 200 && run.json?.data?.ok === true && (run.json?.data?.providers || []).length === 30, `items=${run.json?.data?.item_count}`);

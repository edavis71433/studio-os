...
{
  ok('healthy fixture: all 33 providers executed ok (15 M9.0 + 13 M10 + connected + 4 industry packs)', healthy.record.length === 33 && healthy.record.every((r) => r.ok));
...
// ═══ 8. composability + catalog enforcement ═══
{
  ...
  ok('composability: a 34th provider plugs into the same contract', r.record.length === 34 && r.record[33].ok && r.record[33].items === 1);
...
if (SB && SR && ANON) {
  ...
  ok('integration: all 33 providers executed on staging', (run.json?.data?.providers || []).length === 33 && run.json.data.providers.every((p) => p.ok), (run.json?.data?.providers || []).filter((p) => !p.ok).map((p) => p.name + ':' + p.error).join(';'));

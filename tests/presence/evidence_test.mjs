...
{
  ok('healthy fixture: all 29 providers executed ok (15 M9.0 + 13 M10 + connected)', healthy.record.length === 29 && healthy.record.every((r) => r.ok));
...
// ═══ 8. composability + catalog enforcement ═══
{
  ...
  ok('composability: a 30th provider plugs into the same contract', r.record.length === 30 && r.record[29].ok && r.record[29].items === 1);
...
if (SB && SR && ANON) {
  ...
  ok('integration: all 29 providers executed on staging', (run.json?.data?.providers || []).length === 29 && run.json.data.providers.every((p) => p.ok), (run.json?.data?.providers || []).filter((p) => !p.ok).map((p) => p.name + ':' + p.error).join(';'));

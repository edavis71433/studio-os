...
// PARTIALLY WIRED (Phase T-STARTER, Jul 13 2026): the `industries[]` model on each
// component is now read in production by routes/content.ts (handleBlockSuggestions →
// componentsForIndustry) to order the "Add a section" library recommended-first.
// The rest of this catalog (fields/schema/a11y contracts) remains the frozen-
// architecture spec its render engine + SDK read; do not assume those guard runtime.
...
export const COMPONENTS: ComponentDef[] = [
  ...
  { key: 'carousel', label: 'Carousel', category: 'media', purpose: 'A swipeable slideshow of images with optional captions.',
    fields: [t('image', 'image', { required: true, repeatable: true }), t('caption', 'text', { max: 160 })], schema: null, a11y: 'zero-JS CSS scroll-snap — swipeable + keyboard-scrollable, images have alt', seo: 'visual storytelling', industries: ['*'] },
  { key: 'progress', label: 'Progress bars', category: 'content', purpose: 'Labeled progress or skill bars (a label + a 0–100% level).',
    fields: [t('label', 'text', { required: true, repeatable: true, max: 60 }), t('percent', 'number')], schema: null, a11y: 'each bar is a labeled progressbar with aria-valuenow', seo: 'at-a-glance metrics', industries: ['*'] },

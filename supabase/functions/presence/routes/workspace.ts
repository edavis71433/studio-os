...
export async function handlePortalFeed(jwt: string, site: SiteRow, principal: Principal, cors: Record<string, string>) {
  ...
  if (seesFull) {
    try {
      ...
      for (const r of sup) { const pid = r.project_id ? String(r.project_id) : ''; const c = clientFor(pid, String(r.requester || '')); const profilePid = pid || (c.client_id ? clientToProject[c.client_id] || '' : ''); client_messages.push({ type: 'support', id: r.id, subject: String(r.subject || 'Support request').slice(0, 120), project: pid ? (projById[pid]?.name || '') : '', client_id: c.client_id, client: c.client, created_at: r.updated_at, href: profilePid ? `/crm.html?project=${profilePid}&tab=delivery` : `/projects.html?support=${r.id}` }); }
      for (const [pid, cv] of convo) { const c = clientFor(pid, ''); client_messages.push({ type: 'message', project: projById[pid]?.name || '', client_id: c.client_id, client: c.client, created_at: cv.latest.created_at, needs_reply: (cv.latest.detail || {}).from === 'client', count: cv.count, href: `/crm.html?project=${pid}&tab=delivery` }); }
